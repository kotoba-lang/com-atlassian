(ns atlassian-compat.main
  "Kotodama WASM entrypoint for the Atlassian clean-room actor (L4) — Clojure port.
  ...
  facts (`atlassian.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "atlassian")
(def tier "L4")
...
(def entity-specs
  [{:entity "Project"         :plural "projects"         :id-prefix "atlassia_pro"
    :fields [:key :name :projectTypeKey :style :archived :simplified] :required [:key :name]
    :coerce {:archived :bool :simplified :bool}  :refs {}}
   {:entity "IssueType"       :plural "issuetypes"       :id-prefix "atlassia_iss"
    :fields [:name :description :subtask :hierarchyLevel] :required [:name :description]
    :coerce {:subtask :bool :hierarchyLevel :int}  :refs {}}
   {:entity "Status"          :plural "statuses"         :id-prefix "atlassia_sta"
    :fields [:name :description]                  :required [:name :description]
    :coerce {}                 :refs {}}
   {:entity "StatusCategory"  :plural "statuscategories" :id-prefix "atlassia_sta"
    :fields [:key :name :colorName]               :required [:key :name]
    :coerce {}                 :refs {}}
   {:entity "User"            :plural "users"            :id-prefix "atlassia_use"
    :fields [:accountId :accountType :displayName :emailAddress :active] :required [:accountType :displayName]
    :coerce {:active :bool}    :refs {}}
   {:entity "Comment"         :plural "comments"         :id-prefix "atlassia_com"
    :fields [:body :created :updated :jsdPublic]  :required [:body :created]
    :coerce {:jsdPublic :bool} :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"atlassian.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "atlassian-compat" :tier tier :entities entities} 200])
...
(defn start! [] :atlassian-compat/ready)
